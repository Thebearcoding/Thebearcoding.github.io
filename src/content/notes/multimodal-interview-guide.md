...
- 复习问答
- 图解大模型算法
summary: 按知识依赖串起数学、模型、视觉视频、训练和复习问答的学习入口。
draft: false
obsidian: true
...
这套笔记围绕一个问题组织：**视频中的证据，怎样经过模型变成答案，又怎样通过训练和评估变得可靠？** 学习对象是视觉/视频多模态算法，不是 Agent 框架开发。以余昌叶的 [LLM-RL-Visualized](https://github.com/changyeyu/LLM-RL-Visualized#header-1) 为图解入口，用论文和官方文档补足机制、推导与边界，再通过复习问答与项目推演检验理解。
...
| 阅读入口 | 读完应该能够做到 | 对应练习 |
|---|---|---|
| [读懂大模型公式之前：数学与张量预备课](/notes/multimodal-math-prerequisites/) | 区分矩阵乘法与逐元素乘法，算 softmax、对数概率，解释梯度与 KL | 第 1 天 |
...
图像为原仓库教学图，已保留余昌叶与仓库署名；来源及非商业使用条件见[来源、图像许可与知识点覆盖](/notes/multimodal-sources-coverage/)。高清图保存在本地配图目录，博客版本可点击原图放大。

<span id="mm-f9c93707902f" style="display:block;scroll-margin-top:6rem"></span>

## 3. 用问题地图检查知识是否连通

| 问题主题 | 要检查的能力 | 在哪里展开 |
|---|---|---|
| QKV 实现、位置编码、模型家族差异 | 说明张量形状、运算机制和版本边界 | 01、08 章及 05 章 Q1–Q3 |
| 项目做什么、数据如何做、失败如何解决 | 用任务定义、控制变量和结果支撑结论 | 05 章项目推演，02/06/07 章 |
| Prefill/Decode 瓶颈 | 区分计算、访存、输入处理与生成时延 | 04 章及 05 章 Q4 |
| ViT、CLIP、Q-Former/MLP、MRoPE、蒸馏、视频时间轴 | 串起视觉表示、语言接入、训练目标与时序评价 | 05 章 Q6–Q14，06/08 章 |
| SFT 后 GRPO 不涨、优势估计疑问 | 区分估计器与实现，并定位训练信号失效的环节 | 02、03、07 章 |
| 手算、反例、假设视频实验 | 独立复现数值，检查假设，提出可测对照 | 各章和 14 天练习册 |

本系列的 QA 是按知识点组织的复习问答，不代表某公司的题库或面试频率统计。它们用于检查原理是否能被解释、计算和验证；完整问答与项目推演见[面试问题与项目深挖](/notes/multimodal-interview-questions/)。
...
本版深入展开多模态算法的共同骨架，以及复习与项目分析所需的核心主题。图解仓库用于组织知识入口，本系列并非其逐页改写。Agent 工程、RAG 服务编排、Function Calling、MARL/DDPG/MCTS 等暂不纳入；VideoMAE 已展开 tubelet、tube mask 和重建损失的核心机制，但不等于完整复现；DINOv2 仍是对比与延伸入口。范围逐项列在[来源、图像许可与知识点覆盖](/notes/multimodal-sources-coverage/)。
